...
// Add your own tests    
    ...
    // Sloop fights two encounters, wins one, loses one, gets sunk
    @Test
    public void sloopFightSkirmishThenSink() {
        game.commissionShip("Arrow"); //skill = 5 (for all sloop)
        game.fightEncounter(2); //should win or rest
        game.restoreShip("Arrow");

        String actual = game.fightEncounter(6); // SKIRMISH: battle skill required = 8, so it should lose and sink
        assertTrue(actual.contains("lost on battle skill"));
        assertTrue(game.getSunkShips().contains("Arrow"));
    }
