...
//    
    ...
    //sloop should be able to fight a SKRMISH and either win or lose based on skill
    @Test
    public void sloopCanFightSkirmish() {
        game.commissionShip("Arrow"); // battle skill = 5
        String actual = game.fightEncounter(2); // SKIRMISH needs 3
        assertTrue(actual.contains("won")||actual.contains("lost on battle skill"));
    }
    
    //sloop cannot take part in BLOCKADE encounters
    @Test
    public void sloopCantFightBlockade() {
        game.commissionShip("Arrow");
        String actual = game.fightEncounter(3); // BLOCKADE
        assertTrue(actual.contains("no suitable ship") || actual.contains("no ship available"));
        }
